...
# =============================================================================
# $Id: plugnose.py 74 2007-02-05 22:29:00Z s0undt3ch $
# =============================================================================
#             $URL: http://i18ntoolbox.ufsoft.org/svn/sandbox/nosebitten/nosebitten/plugnose.py $
...
log = logging.getLogger('nose.plugins.plugnose')
...
class BittenNoseCoverage(Plugin):
    ...
    def report(self, stream):
        ...
        coverage_re = re.compile(
            r'(?P<mod>[\w\.]+)\s+(?P<lines>\d+)\s+(?P<exec>\d+)\s+(?P<cover>\d+)%?\s+(?P<miss>[\d,\s-]+)'
        )
